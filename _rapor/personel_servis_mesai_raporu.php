
<?php

include_once("$REAL_P/function/cursorfunction.php");
$nSayfakayit=50;

$nAdonem = tarihtodonemid(time());
$nDonem  = isset($_GET["donem"])?$_GET["donem"]:$nAdonem;
$nSayfa  = isset($_GET["sayfa"])?$_GET["sayfa"]:0;
$nPerso  = isset($_GET["perso"])?$_GET["perso"]:$oUser->perso;
$nOffset = $nSayfa*$nSayfakayit;
$cServis = $oPerso->servis_exp;

$cFields = "perso_exp, tasaat Teorik_Gelis, tcsaat Teorik_Çýkýþ, perso_perno, sum(asaat) Geliþ_Fark, sum(csaat) Çýkýþ_Fark";

$cSqlStr = "select SQL_CALC_FOUND_ROWS $cFields 
                from asist.mesai 
               where mesai.servis=$oPerso->servis 
                 and mesai.donem=$nDonem 
               group by perso_exp 
               order by servis_exp, perso_exp
               limit $nOffset, $nSayfakayit";
               
$curResult = mysqli_query($cDblink, $cSqlStr);

//echo mysqli_error($cDblink);

if ($par_islem=="down")
    cursor_to_excel($curResult,"$cServis ”Servis Mesai Raporu“");
else{
    $objParam->cLink    = "?personel_sirketmesairaporu&donem=$nDonem";
    $objParam->cBaslik  = "$cServis  ”Servis Mesai Raporu“";
    $objParam->nSayfano = $nSayfa;
    $objParam->nMaxrec  = $nSayfakayit;
    $objParam->cGrup    = "";
    $objParam->aLink[]  = array("perso_exp", "?personel_aylikmesailistesi&donem=$nDonem&sayfa=0&perso=%perso");
    $objParam->cHideFlds= "perso_perno,";
    listelehyperlink($curResult, $objParam);
}

for ($ii=$nDonem+1; $ii>=$nDonem-1 && $ii>=$nAdonem; $ii--)
    echo "<a href=\"?personel_servismesairaporu&donem=$ii&sayfa=0\">", donemidtodonem($ii),"</a> &nbsp; ";

?>
