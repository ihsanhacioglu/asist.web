...
<?php
	...
	if(empty($email)){
		echo "--------------------------------------------<br>";
		echo "E-posta listesine girecek e-posta adresi belirtilmemiş.<br>";
		exit;
	}
	...
	if($qList->reccount==0 || strtolower($qList->rec_exp)!=strtolower(trim($email))){
		echo "--------------------------------------------<br>";
		echo "$email<br>E-posta listesine hiç üye olmamış<br>";
		exit;
	}
	...
	if($qList->rec_abc=='A'){
		echo "--------------------------------------------<br>";
		echo "$email<br>$qList->rec_atarih tarihinden beri listeye üye<br>";
		exit;
	}
