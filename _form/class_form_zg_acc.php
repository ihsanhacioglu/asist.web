...
class class_form_zg_acc extends class_form{
	...
	function createCalFlds(){
		...
		$this->arrGun=array();
 		$this->arrGun[]="sonntag";
		$this->arrGun[]="montag";
		$this->arrGun[]="dienstag";
		$this->arrGun[]="mittwoch";
		$this->arrGun[]="donnerstag";
		$this->arrGun[]="freitag";
		$this->arrGun[]="samstag";
		...
		$strSql="select ZKP_Index, ZKP_Standort,
				...
				where ZP.zpnr=?prm_zpnr:S and yy.Id in (1,2,9,10) and ZP.Acmnr+yy.Id2=ZKP.zkp_Index
				order by 1";
		...
		while($qCCC->next()){
			...
			if(!array_key_exists($txt_name,$this->arrCals)){
				...
				$this->arrCals[$txt_name]=$qCCC->rec_biszeit;
 				foreach($this->arrGun as $ii=>$gun){
					$oFld=$qCCC->fieldByName($gun);
					$gun=ucfirst(substr($gun,0,3));
					$val=($oFld->value?1:0);
					$opt=($val?"checked":"");
					$lbl_name="label".$nn++;
					...
					$retStr.="<label class='lbl' style='color:red;width:35'>$gun</label>";
	...
	function updateZP(){
		...
		$strSql="update ZutrittsProfile
				set ".($this->qry->id!=$this->qry->rec_zg_nummer?"zpnr=?prm_zpnr2:S,":"")."
					vonzeit=?prm_vonzeit,
					biszeit=?prm_biszeit,
					[terminal 1]=?prm_t01,
					[terminal 2]=?prm_t02,
					...
					sonntag=?prm_sonntag,
					montag=?prm_montag,
					dienstag=?prm_dienstag,
					mittwoch=?prm_mittwoch,
					donnerstag=?prm_donnerstag,
					freitag=?prm_freitag,
					samstag=?prm_samstag
				where zpnr=?prm_zpnr:S and Acmnr=?prm_acmnr:S";
		$qCCC=new clsAdoacc($this->qry->get_dbLink(), $strSql);
		...
		foreach($arrAcmnr as $acmnr=>$oCal){
			...
			foreach($this->arrGun as $gun)if($oPar=$qCCC->paramByName($gun))$oPar->value=(isset($oCal->$gun)?$oCal->$gun:0);
			$qCCC->exec();
		}
	}
	function insertZP(){
		$strSql="insert into ZutrittsProfile (zpnr,acmnr,vonzeit,biszeit) values
				(?prm_zpnr:S,?prm_acmnr:S,?prm_vonzeit,?prm_biszeit)";
		$strSql="insert into ZutrittsProfile (zpnr,acmnr,vonzeit,biszeit,pincodeeingabe,
				...
				(?prm_zpnr:S,?prm_acmnr:S,?prm_vonzeit,?prm_biszeit,0,
				1,1,1,1,1,1,1)";
	...
	function deleteZP(){
		$strSql="delete from ZutrittsProfile where zpnr=?prm_zpnr:S";
		$qZP=new clsAdoacc($this->qry->get_dbLink(), $strSql);
 		$qZP->prm_zpnr=$this->qry->rec_zg_nummer;
		$qZP->exec();
	}
