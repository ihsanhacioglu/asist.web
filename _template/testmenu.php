<?php
if (!$oUser->roletest && !$oUser->admin) {
	return;
}

// kontrol yapılmadığında Undefined index: QUERY_STRING in C:\asist.web\_template\testmenu.php on line
...
while($oRec=mysqli_fetch_object($res))
	...
	else$str_menu.="<li><a class=\"test\" href=\"?testlogin&role=$oRec->id&qrys=$sQryStr\" title=\"role: $oRec->id\">$oRec->exp</a></li>\n";
...
if(count($oUser->testroles)){
	...
	foreach($oUser->testroles as $role=>$val){
		$str_role.=", ".++$ii.".$val";
		$str_test.="<li><a class=\"test-sel\" href=\"?testlogin&role=$role&qrys=$sQryStr\" title=\"role: $role\">$cKare $val</a></li>\n";
	}
...
if($oUser->admin)
	$str_menu.="<li><a class=\"test\" href=\"?testlogin&roleadmin=".
		($oUser->roleadmin?"0":"1").
		"&qrys=$sQryStr\" title=\"RoleAdmin...$oUser->roleadmin\">".
		($oUser->roleadmin?"++":"--").
		"RoleAdmin...</a></li>\n";
